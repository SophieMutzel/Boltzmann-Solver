subroutine initial_conditions( params, q, q_new, q_tot, rhs )
  ...
  allocate(q_tot(nrhs+5,params%N,L+1))
  allocate(rhs(nrhs+5,params%N,L+1))
  ...
  do i=1,params%N
    ...
    !q(1,i) = neq(T_start, params%mx, gDM)/ent(T_start, params)
    ! Y_a=Yeq,a(T')
    q(2,i) = neq(sqrt(params%gaff(i))*Ta(T_start,params), params%ma, ga)/s
    !q(2,i) = neq(T_start, params%ma, ga)/ent(T_start, params)
  end do
  ! Y_chi
