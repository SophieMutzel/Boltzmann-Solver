...
!!  Parameters:
!!
!!    Input, external real ( kind = rk ) F, the name of the function routine, of the form
!!      function f ( x )
!!      real ( kind = rk ) f
!!      real ( kind = rk ) x
...
!!
!!    Input, real ( kind = rk ) EPSABS, EPSREL, the absolute and relative accuracy requested.
!!
...
!!
!!    Output, real ( kind = rk ) ABSERR, an estimate of || I - RESULT ||.
!!
...
!!    6, the input is invalid, because EPSABS < 0 and EPSREL < 0.
!!
!!    Workspace, real ( kind = rk ) ALIST(LIMIT), BLIST(LIMIT), contains in entries 1
!!    through LAST the left and right ends of the partition subintervals.
!!
!!    Workspace, real ( kind = rk ) RLIST(LIMIT), contains in entries 1 through LAST
!!    the integral approximations on the subintervals.
!!
!!    Workspace, real ( kind = rk ) ELIST(LIMIT), contains in entries 1 through LAST
...
!  return
!end
subroutine qagi ( f, argsint, bound, inf, epsabs, epsrel, result, abserr, neval, ier )
...
!    which evaluates the integrand function.
!
!    Input, real ( kind = rk ) BOUND, the value of the finite endpoint of the integration
...
!
  ...
  integer ( kind = ik ) id
  integer ( kind = ik ) ier
  integer ( kind = ik ) ierro
  integer ( kind = ik ) inf
  integer ( kind = ik ) iord(limit)
  integer ( kind = ik ) iroff1
  integer ( kind = ik ) iroff2
  integer ( kind = ik ) iroff3
  integer ( kind = ik ) jupbnd
  integer ( kind = ik ) k
  integer ( kind = ik ) ksgn
  integer ( kind = ik ) ktmin
  integer ( kind = ik ) last
  integer ( kind = ik ) maxerr
  integer ( kind = ik ) neval
  logical noext
  integer ( kind = ik ) nres
  integer ( kind = ik ) nrmax
  integer ( kind = ik ) numrl2
  real ( kind = rk ) resabs
  real ( kind = rk ) reseps
  real ( kind = rk ) result
  real ( kind = rk ) res3la(3)
  real ( kind = rk ) rlist(limit)
  real ( kind = rk ) rlist2(52)
  real ( kind = rk ) small
!
!  Test on validity of parameters.
!
  ...
  elist(1) = 0.0D+00
  iord(1) = 0
  if ( epsabs < 0.0_rk .and. epsrel < 0.0_rk ) then
    ier = 6
    return
  end if
!
...
!
  ...
  call qk15i ( f, argsint, boun, inf, 0.0d+00, 1.0d+00, result, abserr, defabs, resabs )!
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call qk15i ( f, argsint, boun, inf, a1, b1, area1, error1, resabs, defab1 )
    call qk15i ( f, argsint, boun, inf, a2, b2, area2, error2, resabs, defab2 )
!
...
subroutine qagp ( f, argsint, a, b, npts2, points, epsabs, epsrel, result, abserr, &
  neval, ier )
!*****************************************************************************80
!
...
!  Parameters:
!
!    Input, external real ( kind = rk ) F, the name of the function routine,
!    of the form
!      function f ( x )
!      real ( kind = rk ) f
!      real ( kind = rk ) x
...
!
!    Input/output, real ( kind = rk ) POINTS(NPTS2), contains the user provided interior
...
!
  ...
  do i = 1, nint
    b1 = pts(i+1)
    call qk21 ( f, argsint, a1, b1, area1, error1, defabs, resa )
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call qk21 ( f, argsint, a1, b1, area1, error1, resa, defab1 )
    call qk21 ( f, argsint, a2, b2, area2, error2, resa, defab2 )
!
!  Improve previous approximations to integral and error
!  and test for accuracy.
!
...
subroutine qags ( f, argsint, a, b, epsabs, epsrel, result, abserr, neval, ier )
!*****************************************************************************80
!
...
!
  ierro = 0
  call qk21 ( f, argsint, a, b, result, abserr, defabs, resabs )
!
!  Test on accuracy.
!
  dres = abs ( result )
...
!
    ...
    b2 = blist(maxerr)
    erlast = errmax
    call qk21 ( f, argsint, a1, b1, area1, error1, resabs, defab1 )
    call qk21 ( f, argsint, a2, b2, area2, error2, resabs, defab2 )
!
!  Improve previous approximations to integral and error
!  and test for accuracy.
!
...
!!    Input, real ( kind = rk ) A, B, the limits of integration.
!!
!!    Input, real ( kind = rk ) C, a parameter in the weight function, which must
...
!!    Input, real ( kind = rk ) A, B, the limits of integration.
!!
!!    Input, real ( kind = rk ) C, a parameter in the weight function, which cannot be
!!    equal to A or B.
!!
!!    Input, real ( kind = rk ) EPSABS, EPSREL, the absolute and relative accuracy requested.
!!
...
!!    which evaluates the integrand function.
!!
!!    Input, real ( kind = rk ) A, the lower limit of integration.
!!
!!    Input, real ( kind = rk ) OMEGA, the parameter in the weight function.
!!
...
!  real ( kind = rk ) alist(limit)
!  real ( kind = rk ) blist(limit)
!  real ( kind = rk ) chebmo(maxp1,25)
!  real ( kind = rk ) elist(limit)
!  real ( kind = rk ) epsabs
!  real ( kind = rk ) erlst(limlst)
!  real ( kind = rk ), external :: f
!  integer ( kind = 4 ) ier
...
!  integer ( kind = 4 ) numrl2
!  real ( kind = rk ) omega
!  real ( kind = rk ), parameter :: p = 0.9E+00
!  real ( kind = rk ), parameter :: pi = 3.1415926535897932E+00
!  real ( kind = rk ) p1
!  real ( kind = rk ) psum(52)
!  real ( kind = rk ) reseps
!  real ( kind = rk ) result
!  real ( kind = rk ) res3la(3)
!  real ( kind = rk ) rlist(limit)
...
!!    Input, real ( kind = rk ) A, B, the limits of integration.
!!
!!    Input, real ( kind = rk ) ALFA, BETA, parameters used in the weight function.
...
!!    Input, real ( kind = rk ) A, B, the limits of integration.
!!
!!    Input, real ( kind = rk ) C, the parameter in the weight function.
!!
!!    Output, real ( kind = rk ) RESULT, the estimated value of the integral.
...
!  real ( kind = rk ) cc
!  real ( kind = rk ) centr
!  real ( kind = rk ) cheb12(13)
!  real ( kind = rk ) cheb24(25)
!  real ( kind = rk ), external :: f
!  real ( kind = rk ) fval(25)
!  real ( kind = rk ) hlgth
!  integer ( kind = 4 ) i
!  integer ( kind = 4 ) isym
!  integer ( kind = 4 ) k
...
!  real ( kind = rk ) res24
!  real ( kind = rk ) u
!  real ( kind = rk ), parameter, dimension ( 11 ) :: x = (/ &
...
!!
!!    Output, real ( kind = rk ) RESASC, approximation to the integral of abs(F-I/(B-A)).
!!
!!         on entry and return
...
!!    Input, real ( kind = rk ) A, B, the limits of integration.
!!
!!    Input, real ( kind = rk ) BL, BR, the lower and upper limits of integration.
!!    A <= BL < BR <= B.
!!
!!    Input, real ( kind = rk ) ALFA, BETA, parameters in the weight function.
!!
!!    Input, real ( kind = rk ) RI(25), RJ(25), RG(25), RH(25), modified Chebyshev moments
!!    for the application of the generalized Clenshaw-Curtis method,
!!    computed in QMOMO.
!!
!!    Output, real ( kind = rk ) RESULT, the estimated value of the integral, computed by
...
!!    Output, real ( kind = rk ) ABSERR, an estimate of || I - RESULT ||.
!!
!!    Output, real ( kind = rk ) RESASC, approximation to the integral of abs(F*W-I/(B-A)).
!!
...
!!  Parameters:
!!
!!    Input, real ( kind = rk ) X(11), contains the values of COS(K*PI/24), for K = 1 to 11.
!!
!!    Input/output, real ( kind = rk ) FVAL(25), the function values at the points
...
!!
!!    Output, real ( kind = rk ) CHEB12(13), the Chebyshev coefficients for degree 12.
!!
!!    Output, real ( kind = rk ) CHEB24(25), the Chebyshev coefficients for degree 24.
!!
!  implicit none
...
!
!    Input/output, real ( kind = rk ) EPSTAB(52), the two lower diagonals of the triangular
!    epsilon table.  The elements are numbered starting at the right-hand
!    corner of the triangle.
!
!    Output, real ( kind = rk ) RESULT, the estimated value of the integral.
!
!    Output, real ( kind = rk ) ABSERR, estimate of the absolute error computed from
!    RESULT and the 3 previous results.
!
!    ?, real ( kind = rk ) RES3LA(3), the last 3 results.
!
...
!!    Input, real ( kind = rk ) A, B, the limits of integration.
!!
!!    Input, real ( kind = rk ) OMEGA, the multiplier of X in the weight function.
!!
...
!!    Output, real ( kind = rk ) ABSERR, an estimate of | I - RESULT |.
!!
!!    Output, real ( kind = rk ) RESABS, approximation to the integral of the absolute
!!    value of F.
!!
!!    Output, real ( kind = rk ) RESASC, approximation to the integral | F-I/(B-A) |
!!    over [A,B].
!!
!!  Local Parameters:
!!
...
!  return
!end
subroutine qk15i ( f, argsint, boun, inf, a, b, result, abserr, resabs, resasc )
!*****************************************************************************80
!
...
!    which evaluates the integrand function.
!
!    Input, real ( kind = rk ) BOUN, the finite bound of the original integration range,
!    or zero if INF is 2.
!
...
!
!    Input, real ( kind = rk ) A, B, the limits of integration, over a subrange of [0,1].
!
!    Output, real ( kind = rk ) RESULT, the estimated value of the integral.
...
!    value of F.
!
!    Output, real ( kind = rk ) RESASC, approximation to the integral of the
...
!
  ...
  fval1 = f(tabsc1,argsint)
  if ( inf == 2 ) fval1 = fval1+f(-tabsc1,argsint)
...
!
  ...
  do j = 1, 7
    ...
    fval1 = f(tabsc1,argsint)
    fval2 = f(tabsc2,argsint)
    if ( inf == 2 ) then
      fval1 = fval1+f(-tabsc1,argsint)
      fval2 = fval2+f(-tabsc2,argsint)
    end if
...
!!                       calling program.
!!
!!    ?, real ( kind = rk ) P1, P2, P3, P4, parameters in the weight function
!!
...
!  real ( kind = rk ) reskh
!  real ( kind = rk ) result
!  real ( kind = rk ), external :: w
!  real ( kind = rk ), dimension ( 4 ) :: wg = (/ &
!    1.294849661688697E-01,     2.797053914892767E-01, &
...
!  return
!end
subroutine qk21 ( f, argsint, a, b, result, abserr, resabs, resasc )
!*****************************************************************************80
!
...
!
  ...
  do j = 1, 5
    jtw = 2*j
    absc = hlgth*xgk(jtw)
    fval1 = f(centr-absc, argsint)
    fval2 = f(centr+absc, argsint)
    fv1(jtw) = fval1
    fv2(jtw) = fval2
    fsum = fval1+fval2
    resg = resg+wg(j)*fsum
...
!!  Parameters:
!!
!!    Input, real ( kind = rk ) ALFA, a parameter in the weight function w(x), ALFA > -1.
!!
!!    Input, real ( kind = rk ) BETA, a parameter in the weight function w(x), BETA > -1.
!!
!!           ri     - real
...
!  real ( kind = rk ) resasc
!  real ( kind = rk ) reskh
!  real ( kind = rk ) savfun(21)
!  real ( kind = rk ) w10(5)
!  real ( kind = rk ) w21a(5)
!  real ( kind = rk ) w21b(6)
!  real ( kind = rk ) w43a(10)
!  real ( kind = rk ) w43b(12)
!  real ( kind = rk ) w87a(21)
!  real ( kind = rk ) w87b(23)
!  real ( kind = rk ) x1(5)
!  real ( kind = rk ) x2(5)
!  real ( kind = rk ) x3(11)
!  real ( kind = rk ) x4(22)
...
!    largest error.
!
!    Output, real ( kind = rk ) ERMAX, the NRMAX-th largest error = ELIST(MAXERR).
!
...
!!  Parameters:
!!
!!    Input, real ( kind = rk ) X, the point at which the weight function is evaluated.
!!
!!    Input, real ( kind = rk ) C, the location of the singularity.
!!
!!    Input, real ( kind = rk ) P2, P3, P4, parameters that are not used.
!!
!!    Input, integer ( kind = 4 ) KP, a parameter that is not used.
!!
!!    Output, real ( kind = rk ) QWGTC, the value of the weight function at X.
!!
!  implicit none
...
!!    Input, real ( kind = rk ) X, the point at which the weight function is evaluated.
!!
!!    Input, real ( kind = rk ) OMEGA, the factor multiplying X.
!!
!!    Input, real ( kind = rk ) P2, P3, P4, parameters that are not used.
!!
...
!!    2, W(X) = sin ( OMEGA * X )
!!
!!    Output, real ( kind = rk ) QWGTO, the value of the weight function at X.
!!
!  implicit none
...
!!
!!    Input, real ( kind = rk ) ALFA, BETA, exponents that occur in the weight function.
!!
!!    Input, integer ( kind = 4 ) INTEGR, specifies which weight function is used:
!!    1. W(X) = (X-A)**ALFA * (B-X)**BETA
!!    2, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A)
...
!!    4, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A) * log(B-X)
!!
!!    Output, real ( kind = rk ) QWGTS, the value of the weight function at X.
!!
!  implicit none
