subroutine rhs_contributions_in_n( N, lz, Y, params, argsint, rhs )
  ...
  H = Hub( T, params )
  rhs(3,:) =  abs(-l10*3.0_rk*q(1,:) + l10* ((-sv_xxaa*q(1,:)*q(1,:)+sv_aaxx* q(2,:)*q(2,:))/H))! + gam_xxff/H)
  ...
  !    .and. ((gam_agff(1)+2.0_rk*gam_afgf(1)) < 0.001_rk*sv_aaxx(1)*neqazp(1)*neqazp(1))) then
    rhs(4,:) =  abs(-l10*3.0_rk*q(2,:) + l10*((sv_xxaa*neqzp*neqzp - sv_aaxx*q(2,:)*q(2,:))/H + (gam_agff + 2.0_rk*gam_afgf)/H))
  !else
  ...
  do i = 1, params%N
    ...
    if (Tp(i)>mx) then
      ...
      rhs(7,i) = drhoeq(Tp(i),mx,gDM)+drhoeq(Tp(i),ma,ga)
    else
      ...
      ! p,eq,a(T')
        ...
        rhs(7,i) = (q(1,i)*drhoeqneq( Tp(i), mx ) + drhoeq(Tp(i),ma,ga))
      else
...
!        ! both rho/n(T')
        ...
        rhs(7,1) = (q(1,1)*drhoeqneq( Tp(1), mx ) + q(2,1)*drhoeqneq( Tp(1), ma ))
      end if
