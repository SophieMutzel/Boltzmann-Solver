...
!_______________________________________________________________________
    ...
    CONTAINS
      ...
      SUBROUTINE VODE_F90(F,NEQ,Y,T,TOUT,ITASK,ISTATE,OPTS,params,argsint,J_FCN,G_FCN)
! ..
...
! .. Subroutine Interfaces ..
     !INTERFACE
      !  SUBROUTINE F(NEQ,T,Y,YDOT,params,argsint)
...
!       Call DVODE to do the integration.
        ...
        IF (HOWCALL==1) THEN
          CALL DVODE(F,NEQ,Y,T,TOUT,ITASK,ISTATE,OPTS,JDUMMY,GDUMMY,params,argsint)
        ELSE IF (HOWCALL==2) THEN
          CALL DVODE(F,NEQ,Y,T,TOUT,ITASK,ISTATE,OPTS,J_FCN,G_FCN,params,argsint)
        ELSE IF (HOWCALL==3) THEN
          CALL DVODE(F,NEQ,Y,T,TOUT,ITASK,ISTATE,OPTS,J_FCN,GDUMMY,params,argsint)
        ELSE IF (HOWCALL==4) THEN
          CALL DVODE(F,NEQ,Y,T,TOUT,ITASK,ISTATE,OPTS,JDUMMY,G_FCN,params,argsint)
        END IF
        RETURN
...
!_______________________________________________________________________
      ...
      SUBROUTINE DVODE(F,NEQ,Y,T,TOUT,ITASK,ISTATE,OPTS,JAC,GFUN,params,argsint)
! ..
...
!       Initial call to F. (LF0 points to YH(*,2).)
        LF0 = LYH + NYH
        CALL F(N,T,Y,RWORK(LF0),params,argsint)
        NFE = 1
...
!_______________________________________________________________________
        ...
        IF (MITER/=6 .AND. MITER/=7) THEN
          CALL DVSTEP(Y,RWORK(LYH),NYH,RWORK(LYH),EWT,SAVF,ACOR,WM, &
            IWORK(LIWM),F,JAC,DVNLSD,OPTS%ATOL,ITOL,params,argsint)
        ELSE
          CALL DVSTEP(Y,RWORK(LYH),NYH,RWORK(LYH),EWT,SAVF,ACOR,WM, &
            IWORK(LIWM),F,JAC,DVNLSS28,OPTS%ATOL,ITOL,params,argsint)
        END IF
! *****MA48 build change point. Replace above with these statements.
!     IF (MITER /= 6 .AND. MITER /= 7) THEN
!        CALL DVSTEP(Y,RWORK(LYH),NYH,RWORK(LYH),EWT,SAVF,ACOR,WM, &
...
!       Estimate the second derivative as a difference quotient in f.
        ...
        CALL F(N,T1,Y,TEMP,params,argsint,params,argsint)
        NFE = NFE + 1
...
120     ETA = MAX(ETAMIN,HMIN/ABS(H))
        ...
        HSCAL = H
        TAU(1) = H
        CALL F(N,TN,Y,SAVF,params,argsint)
        NFE = NFE + 1
        IF (BOUNDS) THEN
          DO I = 1, NDX
...
!                   R = MAX(ABS(Y(J)),R0/EWT(J))
              ...
              CALL JACSPDB(F,N,TN,Y,SAVF,WM(1),NRFJACDS, &
                ...
                MAXGRPDS,NGRPDS,JPNTRDS,INDROWDS,params,argsint)
              NFE = NFE + IWKDS(7)
              NJE = NJE + 1
           ELSE
...
!             J1 = 2
              ...
              DO J = 1, N
                 ...
                 Y(J) = Y(J) + R
                 FAC = ONE/R
                 CALL F(N,TN,Y,FTEM,params,argsint)
                 DO I = 1, N
...
!         CALL F(N,TN,Y,WM(3))
          CALL F(N,TN,Y,WM(1),params,argsint)
          NFE = NFE + 1
          DO 10 I = 1, N
            R0 = H*SAVF(I) - YH(I,2)
...
!_______________________________________________________________________
      ...
      SUBROUTINE DVPREPS(NEQ,Y,YH,LDYH,SAVF,EWT,F,JAC,params,argsint)
! ..
...
70      K = 1
        ...
        DO I = 1, N
          ...
          Y(I) = Y(I) + FAC*SIGN(ERWT,Y(I))
        END DO
        CALL F(NEQ,TN,Y,SAVF,params,argsint)
        NFE = NFE + 1
        DO J = 1, N
          IF (K>NZ_ALL) THEN
             IF (LP /= 0) THEN
          ...
          DYJ = SIGN(ERWT,YJ)
          Y(J) = YJ + DYJ
          CALL F(NEQ,TN,Y,FTEMP1,params,argsint)
          NFE = NFE + 1
          Y(J) = YJ
          DO 80 I = 1, N
...
!_______________________________________________________________________
      ...
      SUBROUTINE DVRENEW(NEQ,Y,SAVF,EWT,F,params,argsint)
! ..
...
! Beginning of Jacobian related routines that use MA28
      ...
      SUBROUTINE DVNLSS28(Y,YH,LDYH,SAVF,EWT,ACOR,IWM,WM,F,JAC, &
        NFLAG,ATOL,ITOL,params,argsint)
! ..
...
!       ISTATE = 3.
        IF (ISTATC==1 .OR. ISTATC==3) THEN
          CALL DVPREPS(N,Y,YH,LDYH,DTEMP,EWT,F,JAC,params,argsint)
          ISTATC = 0
        END IF
        JCUR = 0
        HRL1 = H*RL1
               ...
               ELSE
                  ...
                  DO NG = 1, NGP
                    ...
                    DO JJ = JJ1, JJ2
                      ...
                      Y(J) = Y(J) + R
                    END DO
                    CALL F(N,TN,Y,FTEMP1,params,argsint)
                    NFE = NFE + 1
                    DO JJ = JJ1, JJ2
                      J = JGP(JJ)
                      Y(J) = YH(J,1)
...
!          Recompute the sparsity structure.
           CALL DVRENEW(N,Y,SAVF,EWT,F,params,argsint)
           IF (J_IS_CONSTANT) J_HAS_BEEN_COMPUTED = .FALSE.
        END IF
        GOTO 10
...
!_______________________________________________________________________
    ...
    SUBROUTINE JACSP(FCN,N,T,Y,F,FJAC,NRFJAC,YSCALE,FAC,IOPT,WK, &
      LWK,IWK,LIWK,MAXGRP,NGRP,JPNTR,INDROW,params,argsint)
...
!_______________________________________________________________________
    ...
    SUBROUTINE JACSPDB(FCN,N,T,Y,F,FJAC,NRFJAC,YSCALE,FAC,IOPT, &
      WK,LWK,IWK,LIWK,MAXGRP,NGRP,JPNTR,INDROW,params,argsint)
